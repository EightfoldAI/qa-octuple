...
import styles from './PCSMobileJDLoggedOutWrapper.client.module.css';

function PCSMobileJDLoggedOutWrapper(props: PropsWithChildren<AppProps>) {
  ...
  useMemo(() => {
    if (searchParams) {
      const parsedParams = JSON.parse(JSON.stringify(searchParams));
      console.log('Search params', parsedParams);
      const roleIndex: number = parseInt(parsedParams.index, 10) ?? 0;
